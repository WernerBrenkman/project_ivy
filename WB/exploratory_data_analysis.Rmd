...
train$f = as.factor(ifelse(train$Id %in% x_below$Id, 'below', ifelse(train$Id %in% x_above$Id, 'above', NA)))
p = ggplot(train[!is.na(train$f),]) + theme_bw() # ggplot(filter(df, set_id == '1')) + theme_bw()
...
draw_corplot = function(var, scale_x_log10 = FALSE, alpha = .3){
  if(scale_x_log10){  
    p + geom_point(aes_string(x = var, y = 'SalePriceLog', col = 'f'), alpha = alpha) + ggtitle(var) + scale_x_log10()
  }else{  
    p + geom_point(aes_string(x = var, y = 'SalePriceLog', col = 'f'), alpha = alpha) + ggtitle(var)
  }
}
...
As stated earlier there are two clusters of points that can be identified when looking at the scatterplots for **rat_Lot_1stFlr** & **rat_garag_land**. It is also interesting to note the strong linear relationship betwee **PC1** and **SalePrice**

```{r, fig.width=10, fig.asp=.75, echo=FALSE, warning=FALSE, message=FALSE}
p = ggplot(filter(df, set_id == '1')) + theme_bw()
plotlist = lapply(c('NeighborhoodClass', 'LotArea', 'rat_Lot_1stFlr', 'rat_garag_land', 'rat_1stFlr_GrLiv', 'PC1'), function(x){draw_corplot(x)})
multiplot(cols = 3, plotlist = plotlist)
```

```{r, fig.width=10, fig.asp=1, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r, fig.width=10, fig.asp=.25, echo=FALSE, warning=FALSE, message=FALSE}
hc_grps = lapply(2:4, function(x){cutree(hc$average, k=3)})
