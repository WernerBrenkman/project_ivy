...
# Not all are converted correctly
...
ExterQual, BsmtQual, KitchenQual, HeatingQC, FireplaceQu, GarageFinish, CentralAir.

To check the negative relationship to SalePriceLog?
```{r}
C1 = ggplot(train, aes(x = train$ExterQual, y = train$SalePriceLog)) + geom_point() + 
      geom_smooth(method=lm , color="red", se=FALSE)
C2 = ggplot(train, aes(x = train$BsmtQual, y = train$SalePriceLog)) + geom_point() + 
      geom_smooth(method=lm , color="red", se=FALSE)
C3 = ggplot(train, aes(x = train$KitchenQual, y = train$SalePriceLog)) + geom_point() + 
      geom_smooth(method=lm , color="red", se=FALSE)
C4 = ggplot(train, aes(x = train$HeatingQC, y = train$SalePriceLog)) + geom_point() + 
      geom_smooth(method=lm , color="red", se=FALSE)
C5 = ggplot(train, aes(x = train$FireplaceQu, y = train$SalePriceLog)) + geom_point() + 
      geom_smooth(method=lm , color="red", se=FALSE)
C6 = ggplot(train, aes(x = train$GarageFinish, y = train$SalePriceLog)) + geom_point() + 
      geom_smooth(method=lm , color="red", se=FALSE)
C7 = ggplot(train, aes(x = train$CentralAir, y = train$SalePriceLog)) + geom_point() + 
      geom_smooth(method=lm , color="red", se=FALSE)

library(gridExtra)
grid.arrange(C1, C2, C3, C4, C5, C6, C7)
```
Interesting to see that in the above instances, where there are negative relationships, that in most instances where the Quality feature = 1 (indicative of 'Poor' quality) the log of the SalesPrice is higher. 
**(Trying to figure this one out?)**

## Considering non-ordinal categorical features
From the list of of non-ordinal features, there are a coupel that stand out and are worth exploring.
...
```{r}
GKtau(train$GarageType, train$GarageFinish)
table(train$GarageType, train$GarageFinish)

GKtau(train$ExterCond, as.factor(train$SalePriceLog))
```

```{r}
(GKtau(train$Neighborhood, train$MSSubClass))
(GKtau(train$Neighborhood, train$HouseStyle))
(GKtau(train$Neighborhood, train$MSZoning))

print(GKtau(train$MSSubClass, train$HouseStyle))
...
## Split Data
```{r}
# split into test and train again (after in code feature engineering)
# =========================================
train = df %>%
  filter(set_id == '1')
test = df %>%
...
#### Model 
# ---------------------
f = (SalePrice ~ OverallQual + OverallCond) 
lr_model = lm(formula = f, 
               data = train)

test_pred = as.numeric(predict(lr_model, train, type='response') > 0.5)

print(sprintf('Test accuracy: %f perc', mean(test_pred == train$SalePrice)))
...
model_var = c('SalePriceLog', 'TotalBsmtSF', 'NeighPrice', 'OverallQual', 'OverallCond', 'YearBuilt', 'GrLivArea', 'BsmtQual', 'KitchenQual', 'HeatingQC', 'FireplaceQu', 'GarageArea', 'CentralAir', 'MSSubClassPrice', 'LotArea', 'LotShape', 'BsmtFinSF1', 'X1stFlrSF')


#model_lin = train[ , !(names(train) %in% fact_vars)]
model_lin = train[ , model_var]

f = SalePriceLog ~ .#-SalePrice 
lr_model = lm(formula = f,
              data = model_lin)

summary(lr_model)

RSS <- c(crossprod(lr_model$residuals))
MSE <- RSS / length(lr_model$residuals)
RMSE <- sqrt(MSE)
